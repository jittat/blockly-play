...
function highlightVariableSet(varName, value) {
  var divId = 'var_' + varName + '_id';
  if(varName != '') {
    if($('#' + divId).length == 0) {
      $('.variables-list').append('<div class="variables" id="' + divId + '" style="display: inline-block; padding-left: 5px; padding-right: 5px;">' + varName + ': <span class="variable-values">0</span></div>');
    }
    
    $('#' + divId + ' .variable-values').text(value.toString());
    $('#' + divId).css('background-color','#ff7777');
    window.setTimeout(function() {
...
function runCode() {
  ...
  $('.variables-list .variables').detach();
